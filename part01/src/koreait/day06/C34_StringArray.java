...
//											ㄴ기본형 데이터 타입
	//enKey[2] 는 String 참조형 타입
	
	for(int i=0;i<engKey.length;i++)
		System.out.println(engKey[i]); //engKey[i]는 참조형 
	
	String[] koreans= new String[5]; 	// 배열요소의 값이 기본형데이터 저장X, 참조값 저장
										//기본 초기값은? nuii 아무것도 참조하지 않은 상태
	...
	System.out.println("1. koreans 배열요소값이 참조하는 문자열의 길이");
	for(int i=0;i<koreans.length;i++)
		if(koreans[i]!=null)//***
	...
	//break로 for를 중단하지 않고 for를 모두 실행했을 때 같은 문자열이 배열 engKey에 없을 때 입니다.
...
//	
//	for(int i=0;i<engKey.length;i++) {
//		if(find.equals(engKey[i])) {		//일치하는 단어 찾으면 for문 종료하기
//			index=i;
//			break;
//		}
//		
