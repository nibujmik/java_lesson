...
//작성자: 김주빈
	...
	System.out.println(Arrays.toString(numbers)); //초기값 출력
	System.out.println("---------------------------------------------------");
	...
	for(int cnt=0; cnt<6;cnt++){ //로또 번호 6개를 구하기 위한 for문
		...
		System.out.println("k="+k+", number = "+numbers[k]); //선택된 난수 출력
		
		lotto[cnt]=numbers[k]; //numbers배열에서 난수로 뽑힌 인덱스 k의 값이 저장
		
		//k위치의 값을 제거(삭제) : 인덱스 k+1부터 마지막요소까지 왼쪽으로 
		for (int i = k; i < numbers.length-1; i++) {//선택된 로또 번로 제외 -> 중복방지
			numbers[i]=numbers[i+1];                 //선택된 값 삭제
		}
		System.out.println(Arrays.toString(numbers)); // 그 외의  값 출력
		
	}
	System.out.println("최종 선택 숫자: ");
	System.out.println(Arrays.toString(lotto));//최종 선택 값 출력 
